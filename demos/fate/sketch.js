...
function initThings ()
{
  ...
  {
    groups.push( { dx: sin(ii * PI/group_count) + random(-1, 1),
                   dy: cos(ii * PI/group_count) + random(-1, 1),
                   cx: random(width),
                   cy: random(height),
...
function keyPressed()
{
  ...
  {
    group_count = min(6, group_count+1);
    initThings();
  }
  else if ( key === 'a' )
  {
    group_count = max(1, group_count-1);
    initThings();
  }
  else if ( key === 'c' )
  {
  ...
  {
    thing_count = min(10000, thing_count+500);
    initThings();
  }
  else if ( key === ',' )
  {
    thing_count = max(500, thing_count-500);
    initThings();
  }
  else if ( key === 'l' )
  {
    compact = min(15, compact+1);
    initThings();
  }
  else if ( key === 'k' )
  {
    compact = max(0, compact-1);
    initThings();
  }
}